...
class BeaconID : NSObject {
    ...
    public func hexBeaconID() -> String {
        var retval = ""
...
class EddystoneBeaconInfo : NSObject {
    ...
    ///
    class func beaconInfoForUIDFrameData(frameData: NSData, telemetry: NSData?, RSSI: Int) -> EddystoneBeaconInfo? {
        if frameData.length > 1 {
            let count = frameData.length
            var frameBytes = [UInt8](repeating: 0, count: count)
            frameData.getBytes(&frameBytes, length: count)
            ...
            return EddystoneBeaconInfo(beaconID: bid, txPower: txPower, RSSI: RSSI, telemetry: telemetry)
        }
        
        return nil
    }
    
    class func beaconInfoForEIDFrameData(frameData: NSData, telemetry: NSData?, RSSI: Int) -> EddystoneBeaconInfo? {
        if frameData.length > 1 {
            let count = frameData.length
            var frameBytes = [UInt8](repeating: 0, count: count)
            frameData.getBytes(&frameBytes, length: count)
